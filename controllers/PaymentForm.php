	...
	class PaymentForm{
		...
		*/
		public function get($id){
			global $app;
			$sth = $this->PDO->prepare("SELECT * FROM formaPagamento WHERE id = :id");
			$sth ->bindValue(':id',$id);
			$sth->execute();
		...
		*/
		public function new(){
			...
			*/
			$sth = $this->PDO->prepare("INSERT INTO formaPagamento (".implode(',', $keys).") VALUES (:".implode(",:", $keys).")");
		...
		*/
		public function edit($id){
			...
			$sth = $this->PDO->prepare("UPDATE formaPagamento SET ".implode(',', $sets)." WHERE id = :id");
			$sth ->bindValue(':id',$id);
			foreach ($dados as $key => $value) {
				$sth ->bindValue(':'.$key,$value);
			}
		...
		*/
		public function delete($id){
			global $app;
			$sth = $this->PDO->prepare("DELETE FROM formaPagamento WHERE id = :id");
			$sth ->bindValue(':id',$id);
			$app->render('default.php',["data"=>['status'=>$sth->execute()==1]],200); 
		}
	}
